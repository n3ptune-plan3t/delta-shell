...
- Launcher now forces `GDK_BACKEND=wayland` so GTK runs as a native Wayland client and uses Wayland fractional scaling (`wp_fractional_scale_v1`) when compositor/toolkit support is available.
